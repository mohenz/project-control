...
| `archive_store` | `archive_store`, `archive store`, `archive`, `아카이브`, `개인용 아카이브`, `개인용 아카이브 저장소` | `D:\Workspace\archive_store` | React/Vite personal archive storage web app with local PostgreSQL/API and Firebase-ready structure | run: `scripts\start.cmd` / verify: `npm.cmd run build`, `npm.cmd run check:syntax`, `curl.exe -s http://127.0.0.1:5175/api/health` / ports: app `5174`, api `5175`, db `54324` | `states/archive_store_current.md` | `README.md`, `config\.env.example`, `docs\transfer_guide.md`, `docs\firebase_setup.md`, `docs\requirements\프로젝트_아키텍처_및_개발계획서.md` | GitHub: `https://github.com/mohenz/archive-store.git`, deploy: Vercel TBD |
