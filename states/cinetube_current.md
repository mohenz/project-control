...
- 2026-07-05: CineTube 로컬 구동 실패 원인 수정. `scripts/start_local_db.ps1`이 설치된 PostgreSQL 중 최신 버전(18.4)을 자동 선택하면서 PostgreSQL 16으로 초기화된 기존 `local/postgres-data`와 호환되지 않아 DB가 기동 실패하던 문제를 확인했다. 기존 데이터 디렉터리의 `PG_VERSION`을 읽어 같은 버전의 PostgreSQL 설치본을 우선 선택하도록 수정했으며, 데이터 초기화/스키마 변경 없이 PostgreSQL 16, local API `3001`, static server `8080` 구동을 복구했다. 검증 결과 `http://127.0.0.1:3001/health`, `http://127.0.0.1:3001/movies?select=id,title&limit=1`, `http://localhost:8080/index.html` 모두 정상 응답.
- 2026-07-01: CineTube 미커밋 변경사항 21개 파일을 검증 후 커밋/원격 배포 완료. 커밋 `eda99ef Improve CineTube import and gallery workflows`를 `origin/rename/persona_full` 및 프로덕션 기준 `origin/main`에 푸시했으며 두 브랜치가 동일 커밋임을 확인. 검증 결과 `node --check assets\js\shared\admin-page.js`, `node --check assets\js\shared\store.js`, `node --check assets\js\shared\ui.js`, `node --check assets\js\pages\gallery.js`, `python -m py_compile scripts\local_api.py`, `git diff --check` 통과. Vercel 공개 URL `https://cinetube-gray.vercel.app` 200 응답 확인.
- 2026-06-29: Webtoon URL 가져오기 대상에 MangaDNA(`https://mangadna.com/manga/...`)를 추가. `scripts/local_api.py`에서 `mangadna.com` 자동 인식, 브라우저형 fetch 헤더, 제목/평점/Alternative/Author/Artist/Genre/Type/요약/포스터/Chapter 링크 파서를 추가하고 추천작 챕터가 섞이지 않도록 `/manga/{webtoon_id}/chapter-*` 패턴으로 제한. `assets/js/shared/admin-page.js`, `assets/js/shared/store.js`에는 `webtoon_import_site` 기본값과 가져오기 placeholder를 보강하고 `admin/webtoons.html` 캐시 키를 `mangadna-webtoon`으로 갱신. 공통코드 seed/migration 파일에는 `code_value='mangadna'`, 표시명 `MangaDNA`, 정렬순서 `15`를 추가. 검증 결과 `python -m py_compile scripts\local_api.py`, `node --check assets\js\shared\admin-page.js`, `node --check assets\js\shared\store.js`, `git diff --check` 통과. 브라우저 `http://localhost:8080/admin/webtoons.html?verify=mangadna-webtoon`에서 `https://mangadna.com/manga/mamitas` 가져오기 성공, `Mamitas`, rating `3.45`, Alternative `三姐妹 ; Three Sisters (MJJ)`, genre `Drama Romance`, poster URL, Chapter 62건 반영 확인. 실제 저장/DB 마이그레이션 실행은 수행하지 않음.
- 2026-06-29: 관리자 `영화정보` 화면의 `메인전시 여부` 입력을 select에서 토글 버튼 방식으로 변경. `assets/js/shared/admin-page.js`에서 `is_main` boolean 필드만 hidden input + `전시/미전시` 토글 버튼으로 렌더링하고, 클릭/가져오기/setFieldValue/수정모드 값 반영 시 hidden 값과 `aria-pressed`/active 상태가 함께 갱신되도록 `bindToggleButtons`, `updateToggleButtons`를 추가. 기존 저장 payload는 `payload.is_main = payload.is_main === "true"` 흐름을 유지. `assets/css/styles.css`에 토글 버튼 스타일 추가, `admin/movies.html` 캐시 키를 `main-toggle`로 갱신. 검증 결과 `node --check assets\js\shared\admin-page.js`, `git diff --check`, 브라우저 `http://localhost:8080/admin/movies.html?verify=main-toggle`에서 `select[name=is_main]` 없음, hidden 값/토글 active 전환 정상, 콘솔 오류 없음 확인. 저장 submit은 수행하지 않음.
- 2026-06-29: 관리자 `영화정보` 화면의 주연배우 1~4 입력 UI를 목록 선택 + 직접입력 병행 방식으로 개선. `assets/js/shared/admin-page.js`에서 각 배우 슬롯에 select와 직접입력 input을 함께 렌더링하고, 직접입력 값이 있으면 저장 시 기존 배우명을 대소문자 무시로 재사용하거나 없을 경우 `actors` 기본 레코드를 자동 생성해 `movies.actor_ids`에 연결하도록 `normalize()`를 비동기화. `assets/css/styles.css`에 `.actor-picker` 2열/모바일 1열 레이아웃 추가, `admin/movies.html`의 `admin-page.js` 캐시 키를 `actor-direct-input`으로 갱신. 검증 결과 `node --check assets\js\shared\admin-page.js`, `git diff --check`, 브라우저 `http://localhost:8080/admin/movies.html?verify=actor-direct-input` 렌더링, 콘솔 오류 없음 확인. 실제 신규 영화 저장/배우 생성은 데이터 변경 방지를 위해 수행하지 않음.
- 2026-06-27: `https://123av.com/ko/v/mikr-048-uncensored-leaked` 영화 가져오기 실패 원인 확인. Python `urllib`와 PowerShell `Invoke-WebRequest` 모두 TLS/transport 연결 단계에서 원격 호스트가 연결을 강제 종료해 HTML 파싱 전 실패함. `scripts/local_api.py`에 123AV/MissAV 가져오기 fallback을 추가해 원격 HTML 조회가 차단되어도 URL 구조 기준으로 `movie_code`, `title`, `source_url`, `video_url`, `category_name=123AV`, `import_warning=remote_fetch_blocked`를 반환하도록 개선. 검증 결과 해당 URL은 `MIKR-048 Uncensored Leaked` 최소 메타데이터를 반환하고 `python -m py_compile scripts\local_api.py` 통과.
- 2026-06-27: 영화정보 가져오기 대상의 MissAV 도메인 변경을 반영해 `123AV` 기준으로 개선. 내부 호환 `code_value='missav'`는 유지하되 관리자 fallback 표시명, 기본 공통코드 seed, 로컬/Supabase schema seed, schema sync 파일을 `123AV`/`https://123av.com/ko/v/...` 기준으로 변경. `scripts/local_api.py`는 기존 missav 계열 입력 URL을 `https://123av.com`으로 정규화하고 작품번호 입력 시 `https://123av.com/ko/v/{code}`를 생성하도록 정리했으며, `123av.com` 요청은 브라우저형 User-Agent/Referer 헤더로 조회하도록 보강. 기존 DB 공통코드 갱신용 `local/import_site_123av_migration.sql`, `supabase/import_site_123av_migration.sql` 추가. 검증 결과 `python -m py_compile scripts\local_api.py`, `node --check assets\js\shared\admin-page.js`, `node --check assets\js\shared\store.js` 통과. 실제 DB 마이그레이션은 사용자 승인 없이 실행하지 않음.
