...
- latest_deployment: `https://pmotools.vercel.app` — 커밋 `0de7858`(2026-09-10, WBS 통계 3개 화면(Stage 통계 `/wbs/stats`, 업무그룹별 통계 `/wbs/group-stats`, 업무그룹 Task 조회 `/wbs/group-tasks`)의 "상태"(지연/정상) 배지를 옆 컬럼의 지연건수/지연율과 같은 기준(`isWbsItemDelayed` 카운트 > 0)으로 통일 — 기존엔 배지만 계획%/실적% 비교(오늘자 선형보간 페이스)를 써서 지연건수=0인데 지연 배지가 뜨는 등 같은 행 안에서 서로 다른 결과가 나올 수 있었다. `countStatsOf`의 오래된 주석("PMO Daily와 같은 기준")도 수정 — 실제로는 계획건수 필터가 다르다(PMO Daily는 계획종료일==기준일, 여기는 기준일까지 누적, Stage 개요라 의도적으로 다름).) 기준 GitHub 커밋 status `Vercel: success` 확인 후 `/api/health` 200 확인. 로컬 main·GitHub origin/main·프로덕션이 모두 동일 커밋으로 일치. 과거 `Production – projectmgmt` 중복 연결 배포는 실패 상태이며 운영 기준이 아님.
