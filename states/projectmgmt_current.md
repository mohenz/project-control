...
- latest_deployment: `https://pmotools.vercel.app` — 커밋 `51f7029`(2026-09-11, 다른 PC에서 프로덕션 Supabase DB를 로컬로 그대로 복사하는 `scripts/migrate-prod-to-local.ps1` + `docs/프로덕션DB_로컬마이그레이션_가이드.md` 추가. pg_dump로 프로덕션은 읽기만 하고 `--clean --if-exists`로 로컬(`localhost:55432`)에 복원 — 복원 대상이 localhost:55432가 아니면 중단하는 안전장치 포함, 비밀번호는 항상 마스킹 출력. 두 개의 임시 로컬 클러스터로 dump/restore 메커니즘 자체를 실제 검증 후 작성, 프로덕션·기존 로컬 데이터는 건드리지 않음) 기준 GitHub 커밋 status `Vercel: success` 확인 후 `/api/health` 200 확인. 로컬 main·GitHub origin/main·프로덕션이 모두 동일 커밋으로 일치. 이 사이에 다른 세션이 올린 11개 커밋(전역 검색, 비관리자 WBS 권한 제한, Stage 공정율 계산을 건수비에서 계획%/실적%로 수정 등, 커밋 `2401c4a`~`30777fb`)도 fast-forward pull로 반영됨. 과거 `Production – projectmgmt` 중복 연결 배포는 실패 상태이며 운영 기준이 아님.
- **[참고] WBS 통계 기준 전체 검토 완료(2026-09-10)**: 사용자가 제안한 9개 기준(Stage 건수/계획건수/실적건수/공정율/계획%/실적%/지연건수/지연율/상태)을 코드와 대조 검토했고, 사용자가 번호순으로 4가지 결정을 확정했다 — ①계획건수/실적건수는 "오늘까지 누적"(현재 코드) 유지, ②공정율은 "실적÷계획"(현재 코드, 0~100% 캡) 유지, ③지연건수는 "진행중 지연 포함"(현재 코드, `isWbsItemDelayed`) 유지, ④상태 배지만 지연건수 기준으로 통일(이번 커밋에서 처리). 즉 이번 검토로 실제 코드 버그였던 건 ④(상태 배지 불일치) 하나였고, 나머지는 현재 구현이 맞다는 게 확인됐다. 설계 검토 과정 문서는 커밋하지 않았고 대화 로그에만 남아 있다.
- **[참고] 진척율(실적÷목표) 롤업 수식은 그대로다.** `rollupProgress`(가중치 합의 actual/planned 비율)는 여러 화면(WBS 통계/업무그룹별 통계/그룹 Task 조회)에서 공유하는 핵심 계산이라 이번엔 건드리지 않았다 — 수학적으로 개별 leaf가 전부 페이스 이상이면 롤업 비율도 절대 100% 밑으로 안 내려간다(증명 완료), 즉 <100%는 항상 "적어도 하나는 오늘자 선형 목표보다 입력된 진도율이 낮다"는 뜻이며 실제 지연(날짜 기준)과는 별개 개념이다. `/wbs/group-tasks`·`/wbs/group-stats`·`/wbs/stats`의 "지연" 배지도 이 화면과 동일하게 진척율<100% 기준(`rollup.actual < rollup.planned`)을 그대로 쓰고 있어, 같은 오해(실제 지연 없이도 "지연" 배지가 붙음)가 재현될 수 있다 — 사용자가 요청하면 동일한 방식(`isDelayed` 존재 여부 기준)으로 확장 적용을 검토할 것.
- **[참고] 새 PC 셋업 스크립트 검증 방식**: 기존 로컬 DB(비밀번호 인증)와 달리 스크립트가 만드는 신규 클러스터는 `initdb -A trust`라 별도 임시 클러스터(포트 56432)를 띄워 역할/DB 생성 로직만 분리 검증했다(기존 dev DB는 건드리지 않음, 이후 정리함). `psql`/`createdb`에는 전부 `-w`(비밀번호 프롬프트 금지)를 붙여 인증 방식이 다른 클러스터에 실수로 연결해도 무한 대기하지 않고 바로 실패하게 했다.
- **[중단, 2026-09-10] Gemini CLI 위임 배포 워크플로우는 폐기했다.** `scripts/deploy-via-gemini.ps1`, `docs/DEPLOY_WORKFLOW_GEMINI.md`, `package.json`의 `deploy` 스크립트를 모두 삭제(커밋 `5ae057b`). 사유: Gemini API 키가 무료 티어 일일 할당량(20회) 소진으로 push 단계 테스트 실패, 결제 연결해도 재발 가능. Codex CLI로 전환을 검토했으나 `codex exec`의 `workspace-write` 샌드박스가 별도 Windows 계정(`CodexSandboxOffline`)에서 실행돼 `.git` 쓰기 권한이 없어 `git fetch/commit/push`가 막힘 — 우회하려면 `--dangerously-bypass-approvals-and-sandbox`가 필요한데 검증 전 사용자가 중단 지시. **재시도하지 말 것**: 두 경로 모두 이 환경에서 추가 조치(Gemini 결제, Codex 샌드박스 우회 승인) 없이는 근본적으로 막혀 있다.
