...
| `pms` | Next.js App Router + TypeScript 기반 SI/IT 프로젝트관리 웹앱 | planned Next.js dev server `localhost:3000`, Vercel Preview/Production | Supabase Auth + Hosted PostgreSQL + Storage, 프로젝트 멤버십 기반 RLS, 로컬 Supabase CLI | GitHub `mohenz/projectmgmt`, Vercel + 분리된 Supabase staging/production 계획 | `states/pms_current.md` |
