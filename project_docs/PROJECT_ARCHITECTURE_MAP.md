...
| `memorybook` | React/Vite 메모·일정·자료실 프로그램 | Vite `localhost:3030` | Supabase Auth + `memo_states` JSONB + `archive_files` + 비공개 Storage/RLS | Vercel 운영 배포 `https://memorybook-theta.vercel.app` | `states/memorybook_current.md` |
| `epms` | Next.js 16 기반 이커머스 SI PMO/거버넌스 Control Tower, 15개 Screen 분리 | Next.js dev server `localhost:3010` | 현재 mock/local state 프론트엔드 MVP, 향후 로컬 PostgreSQL/Supabase | 로컬 검증 완료, GitHub+Vercel+Supabase 배포는 명시 승인 후 진행 | `states/epms_current.md` |
...
| `shadcn_ui` | shadcn/ui 공식 컴포넌트 Registry + CLI + 문서 모노레포 | pnpm/Turborepo, v4 dev server `localhost:4000` | 로컬 소스·Registry 참조용 | 배포 없음, upstream 추적 | `states/shadcn_ui_current.md` |
